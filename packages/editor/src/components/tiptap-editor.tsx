...
export type AttachmentStorageOps = Omit<
  NotePdfDocContextValue,
  'noteId' | 'userId' | 'attachmentsById' | 'revalidate'
>;
...
export interface TipTapEditorProps {
  ...
  onRefreshNotesList?: () => void;
  onUploadFile?: (file: File) => Promise<NoteAttachment>;
  acceptsFile?: (file: File) => boolean;
  resolveNoteIdFromPath?: (path: string) => string | null;
...
export function TipTapEditor({
  ...
  const canInsertAttachments = Boolean(userId && noteId && onUploadFile);
  ...
  const proEntitledRef = useRef(proEntitled);
  proEntitledRef.current = proEntitled;
  ...
  const attachmentsById = useMemo(() => {
    ...
    for (const a of pendingAttachments) m.set(a.id, a);
    return m;
    ...
    () => ({
      ...
      handleDOMEvents: {
        ...
        click: (_view: EditorView, event: MouseEvent) => {
          ...
          const linkedNoteId = resolveNoteIdFromPath?.(path) ?? null;
          if (linkedNoteId) {
            event.preventDefault();
            event.stopPropagation();
            if (event.metaKey || event.ctrlKey) {
              const absUrl = getAbsoluteNoteUrl?.(linkedNoteId);
              if (absUrl) window.open(absUrl, '_blank', 'noopener,noreferrer');
            } else {
              onNavigateToNoteRef.current?.(linkedNoteId);
            }
            return true;
          }
          event.preventDefault();
    ...
    const syncMention = () => {
      ...
      setMention((prev) => {
        ...
        const selectedIndex = same
          ...
          : 0;
        return { from: trigger.from, query: trigger.query, selectedIndex };
      });
    };
    ...
    (record: NoteAttachment) => {
      ...
      editor
        ...
        .insertContent({
          ...
          attrs: { attachmentId: record.id, filename: record.filename },
        })
        .run();
    },
    [editor],
  ...
  const processFiles = useCallback(
    async (files: FileList | File[] | null) => {
      if (!files || !editor || !onUploadFile) return;
      ...
      try {
        for (const file of list) {
          ...
          try {
            const record = await onUploadFile(file);
  ...
  const mentionFiltered = mention ? filterNoteCandidates(mention.query) : [];
  ...
  const resolvedStorageOps = storageOps ?? noopStorageOps;

  const pdfDocContextValue: NotePdfDocContextValue = useMemo(
    () => ({
      noteId,
      userId,
      attachmentsById,
  ...
  return (
    <NotePdfDocProvider value={pdfDocContextValue}>
      <div className="tiptap-editor">
        ...
        <div
          className={
            isFileDragOver && canInsertAttachments && proEntitled && !uploading
