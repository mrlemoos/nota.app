...
function NoteAudioNodeView(props: NodeViewProps) {
  ...
  const filenameAttr = (props.node.attrs.filename as string) || 'Recording';
  ...
  const handleRemove = useCallback(async () => {
    ...
    try {
      ...
      setActionError(
        e instanceof Error ? e.message : 'Could not remove recording',
      );
    }
  ...
  return (
    <NodeViewWrapper
      as="div"
      className={cn(
        'note-audio-block my-4 rounded-md border border-border/60 bg-muted/20',
        props.selected &&
                ...
                <NotaTooltip>
                  ...
                  <NotaTooltipPortal>
                    <NotaTooltipPositioner side="top" sideOffset={6}>
                      <NotaTooltipPopup>Remove recording</NotaTooltipPopup>
                    </NotaTooltipPositioner>
                  </NotaTooltipPortal>
                </NotaTooltip>
              </div>
